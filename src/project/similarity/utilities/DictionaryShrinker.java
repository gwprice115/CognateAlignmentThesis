	...
	public static void main(String[] args) {
		...
		try {
			...
			while((line1 = br1.readLine()) != null){
				i++;
				String one = SimilarityMeasurer.isolateLex(line1).toLowerCase();
				if(correct1.contains(one) || (i % SHRINK_FACTOR == 0)) {
					out1.println(line1);
				}
			}
			out1.close();
			...
			while((line2 = br2.readLine()) != null){
				i++;
				String two = SimilarityMeasurer.isolateLex(line2).toLowerCase();
				if(correct2.contains(two) || (i % SHRINK_FACTOR == 0)) {
					out2.println(line2);
				}
			}
			out2.close();
